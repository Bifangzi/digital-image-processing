...
% 5. 执行闭运算（先膨胀后腐蚀）
closedImage = imclose(imageToProcess, se);

% 6. 显示结果
figure('Name', '图像闭运算处理结果', 'NumberTitle', 'off');

if exist('binaryImage', 'var')
    % 如果进行了二值化，显示原始灰度图、二值图和闭运算结果
    subplot(1,3,1), imshow(grayImage), title('原始灰度图像');
    subplot(1,3,2), imshow(binaryImage), title('二值图像');
    subplot(1,3,3), imshow(closedImage), title('闭运算后的图像');
else
    % 如果没有二值化，显示灰度图和闭运算结果
    subplot(1,2,1), imshow(grayImage), title('原始灰度图像');
    subplot(1,2,2), imshow(closedImage), title('闭运算后的图像');
end

% 7. 保存结果（可选）
saveChoice = questdlg('是否要保存处理后的图像？', '保存结果', '是', '否', '是');
if strcmp(saveChoice, '是')
    [saveFilename, savePath] = uiputfile({'*.jpg;*.png;*.bmp;*.tif', '图像文件 (*.jpg, *.png, *.bmp, *.tif)'}, '保存处理后的图像', 'closed_image.png');
    if ~isequal(saveFilename, 0)
        imwrite(closedImage, fullfile(savePath, saveFilename));
...
disp('图像闭运算处理完成！');% MATLAB图像闭运算操作脚本
clc;        % 清空命令窗口
clear;      % 清空工作区变量
close all;  % 关闭所有图形窗口
